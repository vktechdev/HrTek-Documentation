...
В случае если файл был загружен в поле {{ .Input.File }}, то в документе отобразится название атрибута файла, в противном случае поле останется пустым. Поле возможно использовать вместе с конструкцией «optional\_row» в таблице, тогда строка будет скрываться, если файл не приложен.