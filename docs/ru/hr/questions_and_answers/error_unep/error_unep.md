...
![Screenshot_149.png](./assets/Settings-ErrorsValidation-2.png)
...
## **Выпуск УНЭП через ЕСИА не зависит от сбоев СМЭВ**

В настройках компании доступен переключатель **Пропускать все ошибки (только для ЕСИА)**. 

Актуально, когда сотрудник выпускает УНЭП через ЕСИА, а СМЭВ в этот момент работает с ошибками. Раньше выпуск останавливался, а теперь при включённой настройке система пропускает проверку СМЭВ и продолжает выпуск.  
...
1. На главной странице выбрать подразделение для приема обращений — «Служба по вопросам гражданства и регистрации иностранных граждан».
