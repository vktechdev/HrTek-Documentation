...
Логика доступа к пользовательскому списку новостей и самим новостям (форме просмотра новости):

1. Для групп «Автор новостей» и «Администратор модуля портал» всё остается без изменений: если их права доступа полностью совпадают с доступом к новости, то они видят новость.

2. Для сотрудников все остается без изменений: если права доступа сотрудника попадают хотя бы в один объект ограничения прав доступа (организация, подразделение, целевая аудитория), то он видит новость.

3. Для пользователей с ролями «Редактор виджета Главные новости» и «Редактор виджета Лента публикаций»: если их права доступа попадают хотя бы в один объект ограничения прав доступа (организация), он видит новость. Для таких пользователей скрыты новости, где организация, на которую назначены им права, попадает в подразделение или целевую аудиторию.
