...
- **Состояние**. Чтобы посмотреть подробное описание каждого состояния, наведите указатель мыши на метку состояния. Для сотрудника доступны следующие состояния: 
    ...
    - **Заявление на ЭЦП**,
    - **Выпуск ЭЦП**,
    - **Ошибка выпуска ЭЦП** (возможные ошибки, которые могут возникнут при выпуске УНЭП, приведены в [статье](/ru/hr/questions_and_answers/unep_release#primery_oshibok_dlya_predstaviteley_kompanii)),
    - **Ручная проверка данных** (подробнее об ошибке в [статье](/ru/hr/questions_and_answers/error_unep)),
    - **Активен**,
    - **Уволен**.
