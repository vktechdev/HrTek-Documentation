...
## Что делать, если не отражаются перенесенные отпуска в календаре рабочего времени?

Перейдите в модуль 1С.

Проверьте, что в 1С проведены документы **Перенос отпусков**, и они передаются в КЭДО.

В КЭДО по сотрудникам должны передаваться данные из поля **Это график** регистра **Очередь отсутствий сотрудников (КЭДО)**.

В поле **Это график** установлена зеленая галочка, если передаются данные по сотрудникам из документов **График отпусков** или **Перенос отпуска**, других условий нет. 

При передаче в КЭДО, значения всех полей из регистра отправляются как есть, без дополнительных обработок.

![](./assets/absence_postponement.png)

Регистр **Очередь отсутствий сотрудников (КЭДО)** не хранит данные постоянно. После отправки данных отсутствия в КЭДО, данные из регистра удаляются. В регистре остаются храниться только данные или неподключенных сотрудников, или сотрудников, по которым есть ошибки заполнения, или если функция выгрузки выключена.

<info>

Для проверки рекомендуем создать документ **График отпусков** на одного сотрудника и посмотреть, как данные будут записаны в регистр и что будет отправлено в КЭДО, в раздел **Рабочее время**

</info>

<warn>

Данные после отправки будут удалены из регистра **Очередь отсутствий сотрудников (КЭДО)**, так что лучше сразу открыть данный регистр с отбором по выбранному сотруднику и смотреть сразу после проведения документа

</warn>

Чтобы уже проведенный график отпусков по всем сотрудникам отображался в КЭДО, необходимо заново записать данные уже проведенного документа. Для этого обратитесь к 1С-специалисту вашей компании, так как это внутренняя техническая процедура и она недоступна рядовым пользователям.

Процедура для запуска:

```1c
vkd_ОбработкаДанныхСервер.ОбновитьДанныеОчередиДокумента(Ссылка на график) 
    ``` 
