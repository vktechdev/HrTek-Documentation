...
Чтобы установить выданный сертификат, выберите его в списке и нажмите кнопку **Установить сертификаты**. 

![Установить сертификаты](./assets/eds4.png)

8. Чтобы избежать случайных подписаний неверными сертификатами, менеджер внедрения VK HR Tek загружает для конкретного подписанта открытый ключ его ЭЦП. Чтобы его выгрузить и передать вашему менеджеру для загрузки, экспортируйте сертификат в формате .p7b (подробнее в [статье](/ru/hr/company/actions_eds/upload_ukep)).
